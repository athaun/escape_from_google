...
public class DefaultRenderer extends Renderer {
    ...
    protected void rebuffer() {
        for (SpriteRenderer sprite : sprites) {
            ...
            Vector2f offset = sprite.gameObject.getComponent(SpriteRenderer.class).getOffset();
            if (offset == null) offset = new Vector2f(0, 0);
            Vector2f[] textureCoordinates = sprite.getTexCoords().clone();

            if (sprite.isFlipped()) {
                Vector2f temp = textureCoordinates[0];
                textureCoordinates[0] = textureCoordinates[3];
                textureCoordinates[3] = temp;

                temp = textureCoordinates[1];
                textureCoordinates[1] = textureCoordinates[2];

                textureCoordinates[2] = temp;
            }
            ...
            for (int i = 0; i < 4; i++) {
                ...
                batch.pushVec2(pos.x + offset.x + scaledX, pos.y + offset.y + scaledY);
